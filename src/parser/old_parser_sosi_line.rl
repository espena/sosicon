...
parseSosiLine( std::string sosiLine )
{
 ...
 /* Current element level */
    ...
    %%{
        ...
        koordinater   = ( ( ( ( float )$see_coord_n . space+ . ( float )$see_coord_o )@resolve_coordinates . space* )+ . ( '...KP' . space+ . ( digits )$see_coord_precision )? );
        ...
        sosi_body := ( [\.]+ ${ sosi_level++; } ) . [^ \.\r\n]+ $see_sosi_element %{ std::cout << "\n"; } . [ \r\n] >{ sosi_level = 0; };

        main := ( [^!]+ ) ${ fhold; fgoto sosi_body; };
